...
int KvFlatBtreeAsync::parse_prefix(bufferlist * bl, prefix_data * ret) {
  ...
  int val_index = 1;
  bool val = false;
  vector<pair<std::string, string> > * current = &ret->to_create;
  pair<std::stringstream, stringstream> this_pair;
  stringstream ts;
  ...
  cout << "\t\t\t" << client_name << "-parse_prefix: dump_ptr set to ts "
      << std::endl;
  for (int i = 1; i < (int)bl->length(); i++) {
    ...
    if (!escape) {
      if (s[i] == '\\') {
	cout << "\t\t\t" << client_name << "-parse_prefix: escape set"
	    << std::endl;
	escape = true;
      }
	...
	cout << std::endl << "\t\t\t" << client_name
	    << "-parse_prefix: dump_ptr set to pair.first: ";
	...
	cout << "\t\t\t" << client_name
	    << "-parse_prefix: dump_ptr set to pair.second: ";
	...
	current->push_back(
	    pair<std::string,string>(this_pair.first.str(),
		this_pair.second.str()));
      }
...
int KvFlatBtreeAsync::cleanup(const prefix_data &p, const int &errno) {
  ...
  case -ENOENT: {
    ...
    for(vector<pair<std::string, string> >::const_iterator it = p.to_delete.begin();
	it != p.to_delete.end(); ++it) {
      if (it->first != "1") {
	librados::ObjectWriteOperation rm;
	if(wait_index < (int)waits.size()) {
    ...
    for(vector<pair<std::string, string> >::const_iterator it = p.to_create.begin();
  ...
  default: {
    ...
    map<std::string,bufferlist> new_index;
    map<std::string, pair<bufferlist, int> > assertions;
    for(vector<pair<std::string, string> >::const_reverse_iterator it =
	p.to_delete.rbegin();
	it != p.to_delete.rend(); ++it) {
      new_index[it->first] = to_bl("0" + it->second);
      assertions[it->first] =
	  pair<bufferlist, int>(to_bl(p.prefix + it->second),
	      CEPH_OSD_CMPXATTR_OP_EQ);
      ...
      cout << "\t\t\t" << client_name << "-cleanup: restoring " << it->second
	  << std::endl;
      cout << "\t\t\t" << client_name << "-cleanup: will assert index contains "
	  << "(" << it->first << "," << p.prefix << it->second
	  << ")" << std::endl;
      cout << "it->second is " << it->second << std::endl;
      if(wait_index < (int)waits.size()) {
	usleep(waits[wait_index++]);
      }
      io_ctx.operate(it->second, &restore); //might fail assert - this is fine
    }
    for(vector<pair<std::string, string> >::const_reverse_iterator it =
	p.to_create.rbegin();
	it != p.to_create.rend(); ++it) {
      librados::ObjectWriteOperation rm;
      rm.remove();
      cout << "\t\t\t" << client_name << "-cleanup: removing " << it->second
	  << std::endl;
      if(wait_index < (int)waits.size()) {
	usleep(waits[wait_index++]);
      }
      io_ctx.operate(it->second, &rm);
    }
    librados::ObjectWriteOperation update_index;
    update_index.omap_cmp(assertions, &err);
    update_index.omap_set(new_index);
...
int KvFlatBtreeAsync::split(const string &obj, const string &high_key,
  ...
  if (err < 0){
    ...
    cout << "\t\treading " << obj << " failed with " << err;
    cout << std::endl;
    return err;
  }
  *ver = obj_aioc->get_version();
  ...
  index_obj_map.insert(pair<std::string,bufferlist>(high_key, to_bl("0" + o2w)));
  ...
  if (err < 0) {
    //This is not an error that should happen, so not fully handling it for now.
...
int KvFlatBtreeAsync::rebalance(const string &o1, const string &hk1, int *ver,
  ...
  if (rebalance) {
    ...
    err = aioc1->get_return_value();
    if (err < 0) {
      //This should not happen, so not fully handling it for now.
  ...
  if (err < 0) {
    ...
    prefix_data p;
    parse_prefix(&prefixed_entries[hk1], &p);
    cleanup(p, -ETIMEDOUT);
    return -ECANCELED;
  }
  ...
  if (err < 0) {
    ...
    un_aioc2->wait_for_safe();
    prefix_data p;
    parse_prefix(&prefixed_entries[hk2], &p);
    cleanup(p, -ETIMEDOUT);
    return -ECANCELED;
  }
...
string KvFlatBtreeAsync::str() {
  ...
  for (int i = 0; i < indexer; i++) {
    its[i] = all_maps[i].begin();
    ret << "|" << string((19 - (string("unwritable? ").length()
	+ 1))/2, ' ');
